...
import { cities, estimate, serviceBySlug } from "@/lib/content";
 ...
 * receives the lead; today it validates, prices the job, and returns a
 * reference so the whole flow, including its failure paths, is real.
 */
export async function submitQuote(
  ...
  if (!beds || beds < 1) errors.bedrooms = "Tell us how many bedrooms, so we can price the job.";
  ...
  const priced = estimate(service, beds, baths);
  const reference = `BB-${city.slice(0, 3).toUpperCase()}-${Math.floor(1000 + Math.random() * 9000)}`;

  return { status: "sent", reference, low: priced?.low, high: priced?.high };
}
