...
The two rejected worlds are on the record: the aqua-bubbles-and-sparkles cleaning page, and its muted-sage wellness opposite. Nothing here is teal, mint, or sage; nothing sparkles; there is no photography anywhere, and the only drawn mark in the system is the hero mop (`components/mop.tsx`), flat, exact, and never shaded or perspectived. The identity itself is no longer drawn here: the supplied lockup and icon set in `/public` are the brand's own artwork and the authority on it.
...
### Primary
- **Lamp Amber** (`amber-500`): the action colour. Primary buttons, the focus ring, the browser accent and caret, the selection highlight on the dusk side, the amber counter in the supplied logo badge, the completed rungs of the quote ladder, and the light in the bloom and the mop's damp trail. It is the only saturated colour in the system.
...
*(Recorded divergence from the direction contract: the contract says amber wears nothing but the action. In the build amber also carries the focus ring, the caret and browser accent, the dusk-side selection highlight, the amber counter in the logo badge, and the ambient bloom and the mop's damp trail. Those are all "the lamp" rather than "an action" — that is the system as built, and it is the boundary to hold.)*
...
- The wordmark is the supplied logo's badge (scaling 110% on hover) beside "Build Bright" in display semibold at `1.0625rem`. `public/logo.png` is a 406x100 lockup whose badge occupies the leading 100x100, so the header crops to it with a square box and `object-cover object-left` — one asset, no derived crop to keep in sync. The rest of that lockup is dark-on-transparent artwork and would vanish on dusk, which is why the header keeps live type and only the footer, on plaster, carries the lockup whole.
...
- **The Pass** (`components/mop.tsx`, `.mop-*` / `.floor-*`): the hero's authored moment, pure CSS with no JavaScript, so there is no hydration flash and `both` fill leaves every element in its finished state. **One** string mop, actually working a floor that is actually dirty when you arrive. Beat one — dull, uneven marks lie along the floor line in the open right half (`--floor`, 51.5–82%), never left of ~46% where the text column is. Beat two — the mop crosses from the left over 1200ms on `--ease-threshold`, fading in before it clears the text column, while the headline, lede, buttons and footnote arrive behind it (`.arrive`, 720ms, staggered by a per-element `--d` of 230/370/500/620ms). Beat three — three unequal passes over one 11400ms cycle, each with its own reach and duration and each advancing down the floor; every pass wipes the marks it reaches, which smear a few pixels the way the head was going and then lift, and leaves a damp sheen behind that dries off over 2600ms. Beat four — marks gone, last sheen dried, and from then on it works clean ground, which is the headline's claim drawn. The whole stage is hidden below 768px.

  **The head lies on the floor and the handle leans over it.** That is the structural reason this is a mop and not the corn broom it replaced. The broom pivoted at the hand, so rotation threw the tips up an arc and every keyframe needed a `(1 - cos angle)` term just to keep them on the floor. Here `.mop-handle` rotates about its own bottom edge — the swivel — which never moves, so floor contact is a property of the construction rather than something compensated frame by frame. `.mop-head` turns about the same point at roughly a twelfth of the handle's angle, which is what a swivel does. The lean runs further than the head travels, which puts the hands at about six-tenths of the head's travel: arm work, not a swinging pole.

  Five animations share the cycle and its delay — the traverse on `.mop-rail`, the lean on `.mop-handle`, the swivel on `.mop-head`, the lift on `.mop-carry`, and the splay on `.mop-strands`, that last one offset by `--strand-lag` (110ms). **The lag is the single most important value in the file:** because the strands run behind the handle, every change of direction makes them trail, catch up and overshoot, which is the only thing that reads as cloth. They also fan wide and flatten under load on each drive, gather on each pull, and hang together in a narrow bundle the moment they leave the floor. A mop cleans on the return too, unlike a broom, so both directions of every pass wipe.

  Because of that, the floor is timed to the mop rather than guessed at: `mop.tsx` holds the keyframe percentages and easings as `PATH` and inverts the same cubic-beziers to work out when the head first touches each mark's and each wet patch's x. Legs are walked in order, so the first pass to reach a point owns it and the damp trail grows rightward across the cycle instead of blinking on every return; the lifted carry-back is flagged and cleans nothing. Change a percentage or an easing in the stylesheet and change the matching `PATH` entry there.

  Three knobs place the scene on `.mop-stage`: `--floor` is the floor line, `--mop-x` is where the head rests at the top of the cycle (it must equal `PATH[0].x`), and `--mop-w` sizes the implement, with `--head-h` derived from it so the strand tips land on the floor at any size. `.mop` and `.mop-rail` are full-width, zero-height rails, which is what lets a `translateX` in percent resolve against the hero rather than against the mop — no JavaScript measures anything. Marks are stratified one per slice of the worked floor rather than randomly scattered, because pure random leaves a bare third and a floor with a hole in it reads as a few specks, not as dirt. They are cold `frost-400` mixed down into the ground: this is the one surface on the page amber deliberately misses, and the grit sitting in them is dulled for the same reason — nothing here may read as sparkle. The damp is amber, because the lamp catches a wet floor. Under `prefers-reduced-motion` the hero renders *finished*: no marks, no sheen, mop standing planted at `rotate(-14deg)` where the pass would have left it. A frozen frame of half-wiped floor would read as a defect.

  The mark is a string mop, drawn flat and exact: a tapered ash handle with a real punched hanging hole (`fill-rule`), a moulded grip, a threaded collar, a swivel yoke with its own punched pin hole, a wide clamping band with two screws, and 52 individual strands in two depth layers. The silhouette is the whole argument — a broom's straw radiates from a narrow ferrule, so it is a triangle on its point; these strands leave the full width of the band, hang, then splay and *lie down* where they meet the floor, so it is a squat bell with a ragged flat bottom. The yoke is drawn on the handle rather than on the head, because it leans with the handle and the head does not; put it on the head and it detaches into a floating tab the moment the handle goes over. Strand variation comes from a deterministic hash, so server and client render the identical path list. Never shaded, never in perspective — what reads as material is that it deforms, and the variation is geometry, not lighting.
