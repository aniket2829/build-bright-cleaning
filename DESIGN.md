...
The two rejected worlds are on the record: the aqua-bubbles-and-sparkles cleaning page, and its muted-sage wellness opposite. Nothing here is teal, mint, or sage; nothing sparkles; there is no photography anywhere, and the only drawn mark in the system is the hero broom (`components/sweep.tsx`), flat, exact, and never shaded or perspectived. The identity itself is no longer drawn here: the supplied lockup and icon set in `/public` are the brand's own artwork and the authority on it.
...
- **The Sweep** (`components/sweep.tsx`, `.broom-*` / `.dust-*`): the hero's authored moment, pure CSS with no JavaScript, so there is no hydration flash and `both` fill leaves every element in its finished state. **One** broom, actually sweeping a floor that is actually dirty when you arrive. Beat one — dust rests scattered along an implied floor line in the open right half (`--floor`, 51–62%), never left of ~46% where the text column is. Beat two — the broom enters already mid-stroke and crosses over 1200ms on `--ease-threshold` while the headline, lede, buttons and footnote arrive behind it (`.arrive`, 720ms, staggered by a per-element `--d` of 230/370/500/620ms), pushing the dust it reaches ahead of it. Beat three — it plants and drives the head across the floor on a 4200ms working cycle (plant, drive, lift, carry back), and the dust it has been pushing gathers at one shared `--pile-x`, resolving as the footnote lands. Beat four — the pile disperses and the next two strokes catch the last five stragglers, after which it works clean ground, which is the headline's claim drawn. A soft amber `broom-trace` marks the ground it just passed over. The whole stage is hidden below 768px.

  **The stroke is a floor sweep, not a mark rocking in place.** The head travels because the body pivots at the hand (`transform-origin: 50% 5%`), so rotation throws the tips roughly ten times further than the hand moves — 63.5% to 75.1% of the hero on every drive. Two rules make it read as work rather than as a pendulum. First, `translate` is written *before* `rotate` so the offset stays in screen space, and every keyframe carries a Y term of `292/325 * (1 - cos angle)`: without it the tips climb an arc and the broom lifts clear of the floor as it leans, which is the tell. The carry-back subtracts a further 2.6% so it rides above the ground instead of dragging the dust home again. Second, per-keyframe `animation-timing-function` gives the push its own commit-and-settle curve and the return a softer one.

  Because of that, the dust is timed to the broom rather than guessed at: `sweep.tsx` mirrors the stroke's durations and easings and inverts the same cubic-beziers to work out when the tips actually reach each mote's x, and every mote travels for exactly as long as the head still has to go, so it is pushed in front of the bristles instead of sliding off on its own clock. Change a timing in the stylesheet and change the matching constant there. `--pile-x` lives only in the stylesheet — motes resolve against it with `calc()`, so the pile and the dust heading for it cannot drift apart.

  `--floor` (how far down the tips reach) is the other knob on `.broom-stage`. Each mote is a full-width, zero-height rail carrying one dot at `--x`, which is what lets a `translateX` in percent resolve against the hero rather than against the 3px dot — no JavaScript measures anything. Motes are flattened ellipses with a size-and-brightness ramp across the band, so they read as grit lying on a plane rather than specks floating in the dark; the pile is a soft mound with individual grit on it, because one smooth shape there reads as a spot of light. Dust is `amber-400` mixed into `frost-200` — lit by the same lamp, never a grey speck. Under `prefers-reduced-motion` the hero renders *finished*: no dust at all, broom standing at `rotate(-13deg)` where the pass would have left it. A frozen frame of settled dust would read as a defect.

  The mark is a corn broom, drawn flat and exact: a tapered ash handle with a real punched hanging hole (`fill-rule`), a metal ferrule, two wire bindings, a stitched band, and 34 individual straw strands that flare to two and a half times the bundle width and end on an uneven, worn edge. Strand variation comes from a deterministic hash, so server and client render the identical path list. Never shaded, never in perspective — the variation is geometry, not lighting.
