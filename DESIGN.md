...
- **The Sweep** (`components/sweep.tsx`, `.broom-*` / `.dust-*`): the hero's authored moment, pure CSS with no JavaScript, so there is no hydration flash and `both` fill leaves every element in its finished state. **One** broom, and a floor that is actually dirty when you arrive. Beat one — dust rests scattered along an implied floor line in the open right half (`--floor`, 52–64%), never left of ~46% where the text column is. Beat two — the broom crosses from the left over 1200ms on `--ease-threshold` while the headline, lede, buttons and footnote arrive behind it (`.arrive`, 720ms, staggered by a per-element `--d` of 230/370/500/620ms). Beat three — every mote the pass reaches travels right into one shared `--pile-x` and gathers as a low pile, resolving as the footnote lands; the pile disperses by 3.5s. Beat four — the broom keeps sweeping on its 5600ms arc and the first two passes catch the last five stragglers, after which it works clean ground, which is the headline's claim drawn. The bristles splay and settle on the arc's own cycle; a soft amber `broom-trace` marks the ground it just passed over. The whole stage is hidden below 768px.

  Two knobs tune the scene to the broom's rest position: `--pile-x` (where the bristle tips land across the hero) and `--floor` (how far down they reach), both on `.broom-stage`. Each mote is a full-width, zero-height rail carrying one dot at `--x`, which is what lets a `translateX` in percent resolve against the hero rather than against the 3px dot — no JavaScript measures anything. Motes are flattened ellipses with a size-and-brightness ramp across the band, so they read as grit lying on a plane rather than specks floating in the dark; the pile is a soft mound with individual grit on it, because one smooth shape there reads as a spot of light. Dust is `amber-400` mixed into `frost-200` — lit by the same lamp, never a grey speck. Under `prefers-reduced-motion` the hero renders *finished*: no dust at all, broom standing at `rotate(-13deg)` where the pass would have left it. A frozen frame of settled dust would read as a defect.
